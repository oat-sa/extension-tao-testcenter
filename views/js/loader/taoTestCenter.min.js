...
;var _ns=".eligibility-import",_fileTypeFilters=["text/csv","application/vnd.ms-excel"],_fileExtFilter=/.+\.(csv)$/,_modalDefaults={width:600};return function(datatable,testCenterId){var config={uploadUrl:helpers._url("import","TestCenterManager","taoTestCenter",{uri:testCenterId})};if(_.isEmpty(testCenterId))throw new TypeError("The eligibility provider needs to be initialized with a test center");var destroyModal=function(){eligibilityImport&&eligibilityImport.getElement()&&(datatable.trigger("reload"),eligibilityImport.getElement().modal("destroy").remove())};var eligibilityImport,initModal=function(modalConfig){var errors,$uploader,$reportContainer
...
,$container),$editorContainer=$(".eligibility-editor-container",$container),$editorImportContainer=$(".eligibility-import-container",$container),testCenterId=$container.data("testcenter"),permissions=!1;$container.data("dacenabled")&&(permissions=$container.data("permissions"));var loading=function(){loadingBar.start()},loaded=function(){loadingBar.stop()},handleError=function(err){loaded(),feedback().error(err.message)},success=function(message){loaded(),feedback().success(message),eligibilityTable.trigger("reload")},eligibilityTable=eligibilityTableFactory(testCenterId,permissions).on("loading",loading).on("loaded",loaded).on("add",function(){var self=this
;eligibilityEditorFactory().add($editorContainer).on("ok",function(eligibility){loading(),eligibilityProvider(testCenterId).addEligibilities(eligibility).then(function(response){loaded(),response.failed&&response.failed.length?feedback().warning(__("The following delivery(ies) are already eligible : ")+response.failed.join(", ")):feedback().success(__("New eligible delivery added")),self.trigger("reload")}).catch(handleError)})}).on("edit",function(eligibilityId,eligibilities){var eligibility=_.find(eligibilities,{uri:eligibilityId});eligibilityEditorFactory().edit($editorContainer,eligibility.delivery.label,eligibility.testTakers).on("ok",function(updated){
loading(),eligibilityProvider(testCenterId).editEligibility({deliveries:[eligibility.delivery.uri],testTakers:updated.testTakers}).then(function(response){success(__("Eligible test takers updated"))}).catch(handleError)})}).on("remove",function(eligibilityId,eligibilities){var eligibility=_.find(eligibilities,{uri:eligibilityId});loading(),eligibilityProvider(testCenterId).removeEligibility({deliveries:[eligibility.delivery.uri]}).then(function(){success(__("Eligible delivery removed"))}).catch(handleError)}).on("shield",function(eligibilityId){loading(),eligibilityProvider(testCenterId).shieldEligibility(eligibilityId).then(function(){success(__(
