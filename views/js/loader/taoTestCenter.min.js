...
"getData","GenerisTree","tao"),deliveriesOrder:"http://www.w3.org/2000/01/rdf-schema#label",deliveriesOrderdir:"asc",isDacEnabled:!1,testTakerDataUrl:url.route("getData","TestCenterManager","taoTestCenter")},config=_.defaults({},module.config(),config);_.isObject(config.testTakerDataUrl)?config.testTakerDataUrl=url.route(config.testTakerDataUrl.action,config.testTakerDataUrl.controller,config.testTakerDataUrl.extension):config.testTakerDataUrl=config.dataUrl;return function(){function buildTestTakerTree(id,url,testTakers,isDacEnabled){return testTakers=_.map(testTakers,"uri"),void 0===isDacEnabled&&(isDacEnabled=config.isDacEnabled),
