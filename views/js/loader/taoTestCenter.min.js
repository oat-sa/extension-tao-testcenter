...
"The eligibility provider needs to be initialized with a test center");var destroyModal=function(){eligibilityImport&&eligibilityImport.getElement()&&(datatable.trigger("reload"),eligibilityImport.getElement().modal("destroy").remove())};var initModal=function(modalConfig){modalConfig=_.defaults(modalConfig||{},_modalDefaults),eligibilityImport&&eligibilityImport.getElement()&&eligibilityImport.getElement().addClass("modal").on("closed.modal",destroyModal).modal(modalConfig),function(){var errors=[],$uploader=$("#upload-container"),$reportContainer=$(".report-container");$uploader.on("upload.uploader",function(e,file,data){return reportFactory({actions:[{
id:"continue",icon:"right",title:"continue",label:__("Continue")}]},data.report).on("action-continue",function(){destroyModal()}).render($reportContainer.find(".report"))}).on("fail.uploader",function(e,file,err){errors.push(__("Unable to upload file %s : %s",file.name,err))}).on("end.uploader",function(){0<errors.length&&feedback().error(function(errors){return"<ul><li>"+errors.join("</li><li>")+"</li></ul>"}(errors),{encodeHtml:!1}),errors=[]}),$uploader.uploader({upload:!0,uploadUrl:config.uploadUrl,fileSelect:function(files,done){var givenLength=files.length;(files=_.filter(files,function(file){var checkType=file.type.replace(/("|')/g,"")
...
eligibilities,{uri:eligibilityId});loading(),eligibilityProvider(testCenterId).removeEligibility({deliveries:[eligibility.delivery.uri]}).then(function(){success(__("Eligible delivery removed"))}).catch(handleError)}).on("shield",function(eligibilityId){loading(),eligibilityProvider(testCenterId).shieldEligibility(eligibilityId).then(function(){success(__("Eligibility needs proctor authorization"))}).catch(handleError)}).on("unshield",function(eligibilityId){loading(),eligibilityProvider(testCenterId).unshieldEligibility(eligibilityId).then(function(){success(__("Eligibility don't need proctor authorization any more."))}).catch(handleError)}).on("import",
