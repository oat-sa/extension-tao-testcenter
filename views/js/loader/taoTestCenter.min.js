...
)||0===stack1)&&(buffer+=stack1),buffer+='\n\n<button class="btn-info small remove" title="'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Remove eligibility",options):helperMissing.call(depth0,"__","Remove eligibility",options)))+'" data-action="remove">\n    <span class="icon-bin"></span> '+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Remove",options):helperMissing.call(depth0,"__","Remove",options)))+"\n</button>\n\n"})}),define("css!taoTestCenterCss/eligibilityTable",[],function(){}),define("taoTestCenter/component/eligibilityTable",[
"jquery","lodash","i18n","ui/component","helpers","tpl!taoTestCenter/component/eligibilityTable/status","tpl!taoTestCenter/component/eligibilityTable/actions","css!taoTestCenterCss/eligibilityTable.css","ui/datatable"],function($,_,__,component,helpers,statusTpl,actionsTpl){"use strict";return function(testCenterId){var eligibilities=[];if(_.isEmpty(testCenterId))throw new TypeError("The eligibility provider needs to be initialized with a test center");return component({},{dataUrl:helpers._url("getEligibilities","TestCenterManager","taoTestCenter",{uri:testCenterId})}).on("render",function(){var self=this;this.$component.on("query.datatable",function(){
self.trigger("loading")}).on("load.datatable",function(e){self.trigger("loaded")}).on("beforeload.datatable",function(e,dataSet){dataSet&&dataSet.data&&(eligibilities=dataSet.data)}).datatable({url:this.config.dataUrl,status:{empty:__("No Eligible Delivery yet"),available:__("Eligible Deliveries"),loading:__("Loading")},tools:[{id:"add",icon:"add",title:__("Add"),label:__("Add"),action:function(){self.trigger("add",eligibilities)}},{id:"import",icon:"import",title:__("Import"),label:__("Import"),action:function(){self.trigger("import",eligibilities)}}],model:[{id:"status",label:"",transform:function(value,row){return statusTpl(row)}},{id:"deliveryLabel",
label:__("Delivery"),transform:function(value,row){return row.delivery.label}},{id:"testTakersCount",label:__("Eligible Test Takers"),transform:function(value,row){return row.testTakers&&row.testTakers.length?row.testTakers.length:0}},{id:"actions",label:__("Actions"),transform:function(value,row){return actionsTpl(row)}}],selectable:!1}).on("click",".actions button",function(e){e.preventDefault();var $button=$(this),itemId=$button.parents("[data-item-identifier]").data("item-identifier"),action=$button.data("action");self.trigger(action,itemId,eligibilities)})}).on("reload",function(){this.$component&&this.$component.datatable("refresh")})}}),define(
"tpl!taoTestCenter/component/eligibilityImport/layout",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="";return buffer+='<div id="eligibility-import">\n    <div class="report-container">\n        <div class="section-header flex-container-full">\n            <h2>',buffer+=(0,this.escapeExpression)((helper=helpers.title)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.title)?helper.call(depth0,{hash:{},data:data}):helper
)+'</h2>\n        </div>\n        <div class="main-container flex-container-full report"></div>\n    </div>\n    <div id="upload-container"></div>\n</div>\n'})}),define("taoTestCenter/component/eligibilityImport",["jquery","lodash","i18n","module","helpers","uri","ui/component","tpl!taoTestCenter/component/eligibilityImport/layout","ui/feedback","ui/report","layout/loading-bar","async","ui/modal","css!taoTestCenterCss/eligibilityEditor","ui/uploader"],function($,_,__,module,helpers,uri,component,layoutTpl,feedback,reportFactory,loadingBar,async){"use strict";var _ns=".eligibility-import",_fileTypeFilters=["text/csv","application/vnd.ms-excel"],
_fileExtFilter=/.+\.(csv)$/,_modalDefaults={width:600};return function(datatable,testCenterId){var eligibilityImport,config={uploadUrl:helpers._url("import","TestCenterManager","taoTestCenter",{uri:testCenterId})};if(_.isEmpty(testCenterId))throw new TypeError("The eligibility provider needs to be initialized with a test center");var destroyModal=function(){eligibilityImport&&eligibilityImport.getElement()&&(datatable.trigger("reload"),eligibilityImport.getElement().modal("destroy").remove())};var initModal=function(modalConfig){var errors,$uploader,$reportContainer;modalConfig=_.defaults(modalConfig||{},_modalDefaults),
eligibilityImport&&eligibilityImport.getElement()&&eligibilityImport.getElement().addClass("modal").on("closed.modal",destroyModal).modal(modalConfig),errors=[],$uploader=$("#upload-container"),$reportContainer=$(".report-container"),$uploader.on("upload.uploader",function(e,file,data){return reportFactory({actions:[{id:"continue",icon:"right",title:"continue",label:__("Continue")}]},data.report).on("action-continue",function(){destroyModal()}).render($reportContainer.find(".report"))}).on("fail.uploader",function(e,file,err){errors.push(__("Unable to upload file %s : %s",file.name,err))}).on("end.uploader",function(){0<errors.length&&feedback().error(
function(errors){return"<ul><li>"+errors.join("</li><li>")+"</li></ul>"}(errors),{encodeHtml:!1}),errors=[]}),$uploader.uploader({upload:!0,uploadUrl:config.uploadUrl,fileSelect:function(files,done){var givenLength=files.length;(files=_.filter(files,function(file){var checkType=file.type.replace(/("|')/g,"");return _.contains(_fileTypeFilters,checkType)||""===checkType&&_fileExtFilter.test(file.name)})).length!==givenLength&&feedback().error("Invalid files have been removed"),async.filter(files,function(file,cb){cb(!0)},done)}})};return eligibilityImport=component({add:function($container,options){return this.on("render",function(){var self=this
...
$container),$editorContainer=$(".eligibility-editor-container",$container),$editorImportContainer=$(".eligibility-import-container",$container),testCenterId=$container.data("testcenter"),loading=function(){loadingBar.start()},loaded=function(){loadingBar.stop()},handleError=function(err){loaded(),feedback().error(err.message)},success=function(message){loaded(),feedback().success(message),eligibilityTable.trigger("reload")};eligibilityTable=eligibilityTableFactory(testCenterId).on("loading",loading).on("loaded",loaded).on("add",function(){var self=this;eligibilityEditorFactory().add($editorContainer).on("ok",function(eligibility){loading(),
eligibilityProvider(testCenterId).addEligibilities(eligibility).then(function(response){loaded(),response.failed&&response.failed.length?feedback().warning(__("The following delivery(ies) are already eligible : ")+response.failed.join(", ")):feedback().success(__("New eligible delivery added")),self.trigger("reload")}).catch(handleError)})}).on("edit",function(eligibilityId,eligibilities){var eligibility=_.find(eligibilities,{uri:eligibilityId});eligibilityEditorFactory().edit($editorContainer,eligibility.delivery.label,eligibility.testTakers).on("ok",function(updated){loading(),eligibilityProvider(testCenterId).editEligibility({deliveries:[
eligibility.delivery.uri],testTakers:updated.testTakers}).then(function(response){success(__("Eligible test takers updated"))}).catch(handleError)})}).on("remove",function(eligibilityId,eligibilities){var eligibility=_.find(eligibilities,{uri:eligibilityId});loading(),eligibilityProvider(testCenterId).removeEligibility({deliveries:[eligibility.delivery.uri]}).then(function(){success(__("Eligible delivery removed"))}).catch(handleError)}).on("shield",function(eligibilityId){loading(),eligibilityProvider(testCenterId).shieldEligibility(eligibilityId).then(function(){success(__("Eligibility needs proctor authorization"))}).catch(handleError)}).on("unshield",
function(eligibilityId){loading(),eligibilityProvider(testCenterId).unshieldEligibility(eligibilityId).then(function(){success(__("Eligibility don't need proctor authorization any more."))}).catch(handleError)}).on("import",function(){eligibilityImportFactory(this,testCenterId).add($editorImportContainer)}).init({}).render($tableContainer)}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
